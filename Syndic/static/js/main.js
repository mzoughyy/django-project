...
const info = {
  date: document.querySelector(".calender-year"),
...
info.date.innerHTML = `${currentMonth} ${currentYear}`;
...
const activate = (i) => {
    ...
    if (!d.classList.contains("disabled")) {
        days.map((d) => d.classList.remove("active", "current"));
        d.classList.add("active");
        // check if current day matches day in calendar
        days.map((d) => {
